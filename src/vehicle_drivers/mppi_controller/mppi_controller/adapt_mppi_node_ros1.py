"""Adapt MPPI node — ROS1 (rospy) interface for the MPPI controller.

Functional twin of ``adapt_mppi_node.py`` (rclpy) — same topic contract,
same control loop, same MPPI math.  ROS plumbing is rospy:

  - rospy.init_node / rospy.spin
  - rospy.get_param('~name', default)
  - rospy.Subscriber / rospy.Publisher / rospy.Timer
  - rospy.loginfo / rospy.logwarn / rospy.loginfo_throttle

Helper code from ``utils.py`` and ``viz.py`` is inlined because both
sibling modules import rclpy/ament at module level.  The pure-Python
``MPPI`` class is imported unchanged — it has no ROS dependency.
...
class AdaptMPPINode:
    def __init__(self):
        ...
        self.require_pacmod_enable = bool(gp('~require_pacmod_enable',  True))
        ...
        wp_csv = str(gp('~waypoints_csv', '')) or default_waypoints_path()
        rospy.loginfo(f'Wp_csv: {wp_csv}')
        self.ref_path = load_waypoints(wp_csv, self.olat, self.olon)

        self.obstacles = demo_positions(
            self.ref_path,
            list(gp('~demo/ped_arc_fractions', [0.8])),
            lateral=0.0,
        )
        self.cones = demo_positions(
            self.ref_path,
            list(gp('~demo/cone_arc_fractions', [0.4])),
            lateral=float(gp('~demo/cone_lateral_offset', 1.5)),
        )
...
def main():
    rospy.init_node('adapt_mppi_node')
    AdaptMPPINode()
    rospy.spin()
